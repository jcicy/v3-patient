...
const getConsultOrder = async () => {
  // ?orderId=xxx   route.query.orderId
  // /xxxx   route.params.路由动态id
...
// 复制
const onCopy = async () => {
  if (!isSupported.value) return showToast('浏览器暂不支持复制功能')
...
<template>
  <div class="consult-detail-page" v-if="item">
    ...
    <div class="detail-order">
      ...
      <van-cell-group :border="false">
        ...
        <van-cell
          title="实付款"
          :value="`￥${item.actualPayment.toFixed(2)}`"
          class="price"
        />
      </van-cell-group>
    </div>
  ...
  <div v-else class="consult-detail-page">
    <cp-nav-bar title="问诊详情" />
    <van-skeleton title :row="4" style="margin-top: 18px" />
    <van-skeleton title :row="4" style="margin-top: 18px" />
  </div>
</template>
